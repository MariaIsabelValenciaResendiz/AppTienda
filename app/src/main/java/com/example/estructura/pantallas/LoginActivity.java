package com.example.estructura.pantallas;
...
import com.example.estructura.modelos.Usuario;
import com.example.estructura.red.ApiService;
import com.example.estructura.red.ClienteRetrofit;
import com.example.estructura.utilidades.GestorConectividad;
import com.example.estructura.utilidades.GestorSesion;
import com.example.estructura.utilidades.MapeadorRoles;
...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle estadoGuardado) {
        ...
        // garantiza por el modo edge-to-edge forzado), nosotros mismos
        // calculamos cuánto espacio ocupa el teclado (IME) y se lo
        // agregamos como "padding" abajo a la pantalla. Así el
        // ScrollView siempre puede desplazarse lo suficiente para
        // mostrar el ícono del ojo y el botón, aunque el teclado esté
        // abierto y haya un banner de error visible.
        View vistaRaiz = findViewById(R.id.vista_raiz_login);
        ViewCompat.setOnApplyWindowInsetsListener(vistaRaiz, (vista, insetsVentana) -> {
            Insets barrasSistema = insetsVentana.getInsets(WindowInsetsCompat.Type.systemBars());
            Insets teclado = insetsVentana.getInsets(WindowInsetsCompat.Type.ime());
            vista.setPadding(
                    barrasSistema.left,
                    barrasSistema.top,
                    barrasSistema.right,
                    Math.max(barrasSistema.bottom, teclado.bottom)
            );
            return insetsVentana;
        });
